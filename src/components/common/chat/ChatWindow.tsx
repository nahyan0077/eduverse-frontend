...
export const ChatWindow: React.FC<ChatWindowProps> = ({
	...
	const handleSendMessage = async () => {
		if (inputMessage.trim() || selectedFile) {
			const messageContent = selectedFile ? cloudURL : inputMessage;
			const messageType = selectedFile ? selectedFile.type.split('/')[0] : "text";

			onSendMessage({ content: messageContent, contentType: messageType });
			setInputMessage("");
			setSelectedFile(null);
			setCloudURL(""); // Clear the cloud URL after sending the message
		}
	};
