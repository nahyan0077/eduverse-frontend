...
export const AdminCourses: React.FC = () => {
	...
	const fetchCourses = async () => {
		...
		if (getAllCourseAction.fulfilled.match(result)) {
			...
			const reqCourses = coursesData.filter(
				(course) => course.isRequested === true
			);
								...
								<tbody>
									{courses.map((data, index) => (
										<tr
											key={data._id}
											onClick={() =>
												navigate("/admin/single-course", { state: { data } })
											}
										...
										>
											...
											<td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
												{data.isBlocked ? (
													<span>
														{" "}
														<CheckCircleIcon color="error" /> Blocked{" "}
													</span>
												) : data.isRequested ? (
													<span>
														{" "}
														<HelpIcon color="primary" /> Requested{" "}
													</span>
												) : data.isPublished ? (
													<span>
														{" "}
														<CheckCircleIcon color="success" /> Published{" "}
													</span>
												) : (
													<span>
														{" "}
											...
											<td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
												{data?.isRequested ? (
													<span>
														{" "}
														<PendingIcon color="warning" /> Pending{" "}
													</span>
												) : (
													<span>
														{" "}
														<ChecklistIcon color="success" /> Verified{" "}
													</span>
												)}
											</td>
										...
										>
											...
											<td className="px-6 py-4">
												<button
													onClick={(e) => {
														e.stopPropagation();
														navigate("/admin/single-course", {
															state: { data },
														});
													}}
													className="btn font-medium btn-outline btn-accent btn-sm"
												>
													Approve/Reject
												</button>
											</td>
